  ...
  <MainLayout>
    <div class="py-8">
      <div class="container">
        <!-- 页面标题 -->
        <div class="mb-8">
          <h1 class="text-3xl font-bold text-gray-900 mb-2">编辑提示词</h1>
          <p class="text-gray-600">修改您的提示词内容</p>
        </div>

        <div v-if="promptsStore.isLoading && !prompt" class="animate-pulse">
          <div class="h-8 bg-gray-200 rounded w-1/3 mb-4"></div>
          <div class="h-4 bg-gray-200 rounded w-1/2 mb-8"></div>
          <div class="h-64 bg-gray-200 rounded"></div>
        </div>

        <div v-else-if="prompt" class="max-w-4xl">
        ...
        <div v-else class="text-center py-16">
          ...
          <p class="text-gray-500 mb-6">您要编辑的提示词可能已被删除或不存在</p>
          <router-link to="/my-prompts" class="btn btn-primary">
            返回我的提示词
          </router-link>
        </div>
      </div>
...
const loadPrompt = async () => {
  try {
    ...
    if (prompt.value) {
      form.title = prompt.value.title
      form.categoryId = prompt.value.categoryId
